...
for (vars in 1:length(var_model_vars)) {
  ...
  if (model_name %in% c("VAR_I", "VAR_II", "VAR_III")) {
    ...
    data_var <- data_quarter_d[c("date", var_model_vars[[vars]])] %>%
    ...
    coef_prior <- sapply(vars_names, function(name) {
      as.numeric(arima(data_quarter_d[[name]], order = c(1, 0, 0))$coef[1])
    })
    ...
    data_var <- data_quarter_l[c("date", var_model_vars[[vars]])] %>%
      column_to_rownames("date")
    ...
    df_tmp <- tibble(
      ...
      component = 'p_livre',
      horizon   = 1:12,
      mean      = diff(c(tail(data_quarter_l$p_livre,1), forecast_vec$fcst[['p_livre']][, "fcst"])),
      lower     = diff(c(tail(data_quarter_l$p_livre,1), forecast_vec$fcst[['p_livre']][, "lower"])),
      upper     = diff(c(tail(data_quarter_l$p_livre,1), forecast_vec$fcst[['p_livre']][, "upper"]))
    )
    forecast_df <- bind_rows(forecast_df, df_tmp)
...
saveRDS(forecast_df, file = "data/forecast_quarter.rds")
